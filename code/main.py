...
def get_historic_data(station_name, X, lookback=3):
    ...
    closest_stations = find_closest_stations(distance_matrix, station_name)

    # Iterate through the specified lookback timestamps (30, 60, and 90 minutes)
    for timestamp in range(30, 91, 30):
        # Create a copy of all_data and modify the timestamps
        shifted_all_data = all_data.copy()
        shifted_all_data["timestamp"] += timedelta(minutes=timestamp)

        # Merge the shifted data with the input data X based on the nearest timestamp
        merged_data = pd.merge_asof(
            X, shifted_all_data, on="timestamp", direction="nearest"
        )

        # Add the target station's historic data
        X[f"attr_y{timestamp}"] = merged_data[station_name]

        # Add the historic data for the closest stations
        for i, closest_station in enumerate(closest_stations, 1):
            X[f"attr_y{timestamp}_closest_{i}"] = merged_data[closest_station]

        # Sum the bicycle counts for all stations at each timestamp
        shifted_all_data["total_bicycles"] = shifted_all_data.select_dtypes(
            include=["number"]
        ).sum(axis=1)

        # Merge the total bicycle counts with the input data X based on the nearest timestamp
        merged_total_bicycles = pd.merge_asof(
            X,
            shifted_all_data[["timestamp", "total_bicycles"]],
            on="timestamp",
            direction="nearest",
        )

        # Add the total bicycle count as a new feature for the specified lookback timestamp
        X[f"attr_total_bicycles_{timestamp}"] = merged_total_bicycles["total_bicycles"]

        print(X[f"attr_y{timestamp}"])

    columns_to_transform = (
        [f"attr_y{timestamp}" for timestamp in range(30, 91, 30)]
        + [
            f"attr_y{timestamp}_closest_{i}"
            for timestamp in range(30, 91, 30)
            for i in range(1, 4)
        ]
        + [f"attr_total_bicycles_{timestamp}" for timestamp in range(30, 91, 30)]
    )
...
def load_and_preprocess_data(
    ...
    for st_train, st_test in zip(train_set.columns, test_set.columns):
        ...
        X_train["attr_hour"] = pd.Categorical(
            X_train["attr_hour"], categories=range(24)
        )
        X_test["attr_hour"] = pd.Categorical(X_test["attr_hour"], categories=range(24))

        X_train["attr_minute"] = pd.Categorical(
            X_train["attr_minute"], categories=range(60)
        )
        X_test["attr_minute"] = pd.Categorical(
            X_test["attr_minute"], categories=range(60)
        )

        X_train["attr_weekday"] = pd.Categorical(
            X_train["attr_weekday"], categories=range(7)
        )
        X_test["attr_weekday"] = pd.Categorical(
            X_test["attr_weekday"], categories=range(7)
        )

        # Apply one-hot encoding
        X_train = pd.get_dummies(
            X_train, columns=["attr_hour", "attr_minute", "attr_weekday"]
        )
        X_test = pd.get_dummies(
            X_test, columns=["attr_hour", "attr_minute", "attr_weekday"]
        )
...
def main():
    ...
    if TRAIN_PHASE:
        ...
        for station in stations:
            ...
            if model == "lr":
                # Remove first five rows from x train
                ...
                grid_search = RandomizedSearchCV(
                    rf, rf_params, cv=5, scoring="neg_mean_absolute_error", n_jobs=-1
                )
        ...
        for station in stations:
            ...
            y_train = y_train.values.ravel()
            reg = LinearRegression().fit(X_train, y_train)
            # reg = XGBRegressor(booster="gblinear", feature_selector="shuffle",
            #                   eval_metric="mae", n_estimators=350,
